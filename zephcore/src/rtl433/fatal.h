...
#ifdef SNF_RTL433
/* SNF_RTL433 (ZephCore sniffer): no libc heap, no aborting the firmware.
 * These macros only fire on alloc failure; the static-arena allocator
 * (rtl433_mem.h) signals exhaustion by returning NULL and the callers
 * already handle NULL.  Keep the TU compiling without fprintf/exit. */
#define STRINGIFYX(x) #x
#define STRINGIFY(x) STRINGIFYX(x)
#define FILE_LINE __FILE__ ":" STRINGIFY(__LINE__)
#define FATAL(what) ((void)0)
#define FATAL_MALLOC(what) ((void)0)
#define FATAL_CALLOC(what) ((void)0)
#define FATAL_REALLOC(what) ((void)0)
#define FATAL_STRDUP(what) ((void)0)
#define WARN(what) ((void)0)
#define WARN_MALLOC(what) ((void)0)
#define WARN_CALLOC(what) ((void)0)
#define WARN_REALLOC(what) ((void)0)
#define WARN_STRDUP(what) ((void)0)
#else
#define STRINGIFYX(x) #x
#define STRINGIFY(x) STRINGIFYX(x)
#define FILE_LINE __FILE__ ":" STRINGIFY(__LINE__)
