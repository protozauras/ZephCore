...
#include "data.h"
#ifdef SNF_RTL433
/* SNF_RTL433: Zephyr minimal libc has no <sys/time.h>; we only need the
 * struct as an opaque pointer type (pulse_data_load is not used by the
 * sniffer firmware).  Forward-declare instead of pulling compat_time.h. */
struct timeval;
#else
